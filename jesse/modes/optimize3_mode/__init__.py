...
from jesse.modes.backtest3_mode import load_candles, simulator
...
from jesse import init_worker, run_worker
# for macOS only
...
class Optimizer(Genetics):
    def __init__(self, training_candles: ndarray, testing_candles: ndarray, optimal_total: int, cpu_cores: int, csv: bool,
                 json: bool, start_date: str, finish_date: str, app: Celery) -> None:
    ...
    def fitness(self, dna: str) -> tuple:
        from jesse import run_worker
        
        try:
            ret = _app.send_task("jesse.run_worker", [dna])
            result_output = ret.get()
            # print(f"Remote fitness")
            # jh.dump(result_output)
            return result_output
            #        return run_worker.apply([dna])
        except run_worker.OperationalError as exc:
            print(f' Fitness error {exc}')
            return []


    def _fitness(self, dna: str) -> tuple:
...
def optimize3_init_worker(start_date: str, finish_date: str, optimal_total: int) -> None:
    global optimizer
    # validate routes
    ...
    if not _app:
        ...
        _app = Celery('jesse',  broker=broker_url, backend=backend_url)
    ...
    optimizer = Optimizer(training_candles, testing_candles, optimal_total, cpu_cores, csv, json, start_date, finish_date, _app)

def optimize3_run_worker(dna: str) -> None:
    global optimizer
    if not optimizer:
        raise ValueError('Optimizer is not initialized')
    return optimizer._fitness(dna)


def _remote_init_worker(start_date: str, finish_date: str, optimal_total: int):
    global _app
    ret =  _app.send_task("jesse.init_worker", [start_date, finish_date, optimal_total])
    result_output = ret.get()

def optimize3_mode(start_date: str, finish_date: str, optimal_total: int, cpu_cores: int, csv: bool, json: bool) -> None:
# def optimize3_mode(*args, **kwargs) -> None:
    ...
    cores = 0
    pool = []
    for key in _app.control.inspect().stats().keys():
        print(f"Worker {key} has {_app.control.inspect().stats()[key]['pool']['max-concurrency']} cores")
        pool.append(key)
        cores += _app.control.inspect().stats()[key]['pool']['max-concurrency']
    print(f"Initing workers...{cores}")
    _remote_init_worker(start_date, finish_date, optimal_total)

    ret = _app.send_task("jesse.init_worker", [start_date, finish_date, optimal_total])
    result_output = ret.get()
    rets = []
    for i in range(cores):
        print(f"Initing worker {i}")
        ret = _app.send_task("jesse.init_worker", [start_date, finish_date, optimal_total])
        rets.append(ret)
