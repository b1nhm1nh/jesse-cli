...
from jesse.modes.backtest2_mode import load_candles, simulator
...
def optimizewf_mode(start_date: str, finish_date: str, optimal_total: int, cpu_cores: int, csv: bool, json: bool, train_month: int, test_month: int, inc_month: int) -> None:
    # clear the screen
    click.clear()
    ...
    while  i_finish_date <= a_finish_date:
        print (f" {i_start_date} - {i_finish_date} ")
        i_start_date = i_start_date.shift(months = inc_month)
        i_finish_date = i_start_date.shift(months = train_month + test_month)


        # load historical candles and divide them into training
        # and testing candles (33% for test, 66% for training)
        training_candles, testing_candles = get_training_and_testing_candles(i_start_date.format('YYYY-MM-DD'), i_finish_date.format('YYYY-MM-DD'), train_month)

        optimizer = Optimizer(training_candles, testing_candles, optimal_total, cpu_cores, csv, json, i_start_date.format('YYYY-MM-DD'), i_finish_date.format('YYYY-MM-DD'))
...
def get_training_and_testing_candles(start_date_str: str, finish_date_str: str, train_month: int) -> Tuple[
    ...
    divider_index = jh.date_diff_in_days(jh.timestamp_to_arrow(start_date), jh.timestamp_to_arrow(start_date).shift(months=train_month)) * 1440
    print(f" {days_diff} {divider_index}")
    #int(days_diff * 0.85) * 1440
    for key in candles:
