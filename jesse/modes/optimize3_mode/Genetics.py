...
class Genetics(ABC):
    def __init__(self, iterations: int, population_size: int, solution_len: int,
                 ...
                 options: Dict[str, Union[bool, Any]] = None, app: Celery = None) -> None:
                ...
                with Manager() as manager:
                    ...
                    try:
                        ...
                        for _ in range(self.cpu_cores):
                            ...
                            if all(dna_tuple[0] != dna for dna_tuple in dna_bucket):
                                ret = self._app.send_task("jesse.run_worker", [dna])
                                rets.append(ret)

                        # wait for results
                        for ret in rets:                            
                            fitness_score, fitness_log_training, fitness_log_testing = ret.get()
                            dna_bucket.append((dna, fitness_score, fitness_log_training, fitness_log_testing))
                ...
                with Manager() as manager:
                    ...
                    try:
                        ...
                        for _ in range(self.cpu_cores):
                            ...
                            try:
                                # check if already exists and then return it
                                baby = next(item for item in self.population if item["dna"] == dna)
                                print(f"making love: baby exist")
                                rets.append([None, baby])
                                #return next(item for item in self.population if item["dna"] == dna)
                            except StopIteration:
                                # not found - so run the backtest
                            #    fitness_score, fitness_log_training, fitness_log_testing = self.fitness(dna)
                        ...
                        for retdict in rets:
                            ...
                            try:
                                # check if already exists and then return it
                                baby = next(item for item in self.population if item["dna"] == dna)
                                print(f"mutating: baby exist")
                                babies.append([None,baby])
                            except StopIteration:
                                # not found - so run the backtest
                                ret = self._app.send_task("jesse.run_worker", [dna])
                                print(f"Mutating...")
                                babies.append([ret,dna])
                            
                        
                        for retdict in babies:
                            ret, dna = retdict
                            if ret is not None:
                                fitness_score, fitness_log_training, fitness_log_testing = ret.get()
                                baby = {
